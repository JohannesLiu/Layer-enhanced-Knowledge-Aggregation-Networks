...
class TransE:
    ...
    def transE(self, cI = 20):
        ...
        for cycleIndex in range(cI):
            ...
            if cycleIndex % 100 == 0:
                ...
                print(self.loss)
                self.writeRelationVector("D:/PycharmProjects/LeKAN/embeddings/transe/relationVector_" + str(out_dim) +".txt")
                self.writeEntilyVector("D:/PycharmProjects/LeKAN/embeddings/transe/entityVector_" + str(out_dim) +".txt")
                self.loss = 0
    ...
    def update(self, Tbatch):
        ...
        for tripletWithCorruptedTriplet in Tbatch:
            if tripletWithCorruptedTriplet[0][2] not in copyRelationList:
                continue
...
if __name__ == '__main__':
    dirEntity = "D:/PycharmProjects/LeKAN/RawTrans/NYT-10/entity2id.txt"
    entityIdNum, entityList = openDetailsAndId(dirEntity)
    dirRelation = "D:/PycharmProjects/LeKAN/RawTrans/NYT-10/relation2id.txt"
    relationIdNum, relationList = openDetailsAndId(dirRelation)
    dirTrain = "D:/PycharmProjects/LeKAN/RawTrans/NYT-10/train.txt"
    tripleNum, tripleList = openTrain(dirTrain)
    print("打开TransE")
    transE = TransE(entityList,relationList,tripleList, margin=1, dim = out_dim)
    print("TranE初始化")
    transE.initialize()
    transE.transE(500000)
    transE.writeRelationVector("D:/PycharmProjects/LeKAN/embeddings/trainse\\relationVector_" + str(out_dim) +".txt")
    transE.writeEntilyVector("D:/PycharmProjects/LeKAN/embeddings/transe/entityVector" + str(out_dim) + ".txt")
